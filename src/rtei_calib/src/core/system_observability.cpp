    ...
    cv::Mat LMEquation::EquationGraph(const Eigen::MatrixXd &hMat, const Eigen::VectorXd &bVec) const {
        ...
        for (int r = 0; r < rows; ++r) {
            for (int c = 0; c < rows; ++c) {
                ...
                if (val > 0.0) {
                    float hue = static_cast<float>(std::log2(val + 1) / std::log2(maxVal + 1));
                    color = cv::Scalar::all(1.0) * hue;
//                    color = ns_viewer::Colour::Hsv(
//                            0.0f, static_cast<float>(std::log2(val + 1) / std::log2(maxVal + 1)), 1.0f);
                } else {
                    float hue = static_cast<float>(std::log2(-val + 1) / std::log2(-minVal + 1));
                    color = cv::Scalar::all(1.0) * hue;
//                    color = ns_viewer::Colour::Hsv(
...
//                    .setTo(cv::Scalar(color.b * 255.0, color.g * 255.0, color.r * 255.0));
        ...
        for (const auto &[pName, pDime]: _paramDesc) {
            // HSV颜色值
            float hue = 0.0f; // 色相保持不变
            float saturation = 0.0f; // 饱和度保持不变
            float value = static_cast<float>(cr) / static_cast<float>(cols) + 0.25f; // 亮度根据当前行计算

            // 将HSV转换为BGR
            cv::Scalar bgr;
            cv::cvtColor(cv::Scalar(hue, saturation, value), bgr, cv::COLOR_HSV2BGR);
