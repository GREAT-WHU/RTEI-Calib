...
#ifndef RTEI_CALIB_TYPE_DEFINE_HPP
#define RTEI_CALIB_TYPE_DEFINE_HPP
...
// FMCW LiDAR 
struct EIGEN_ALIGN16 PointXYZTV {
    PCL_ADD_POINT4D;              // quad-word XYZ
    double timestamp;             // laser timestamp
    double velocity;              // Doppler velocity
    PCL_MAKE_ALIGNED_OPERATOR_NEW // ensure proper alignment
};

POINT_CLOUD_REGISTER_POINT_STRUCT(PointXYZTV,                              //
                                  (float, x, x)                           //
                                          (float, y, y)                   //
                                          (float, z, z)                   //
                                          (double, timestamp, timestamp)  //
...
namespace ns_eicalib {
    ...
    using PosTVPoint = PointXYZTV;
    using PosTVPointCloud = pcl::PointCloud<PosTVPoint>;
...
#endif //RTEI_CALIB_TYPE_DEFINE_HPP
