...
#ifndef RTEI_CALIB_BSPLINE_2D_EXAM_HPP
#define RTEI_CALIB_BSPLINE_2D_EXAM_HPP
...
namespace ns_eicalib {
    struct Bezier {
    public:
        static ns_eicalib::aligned_vector<Eigen::Vector2d>
        solve(const ns_eicalib::aligned_vector<Eigen::Vector2d> &controlPoints, std::size_t num) {
            double t = 0.0, delta = 1.0 / static_cast<double>(num - 1);
            ns_eicalib::aligned_vector<Eigen::Vector2d> result(num);
    ...
    protected:
        static Eigen::Vector2d bezier(double t, const ns_eicalib::aligned_vector<Eigen::Vector2d> &controlPoints,
    ...
    struct UniformBSpline {
        static ns_eicalib::aligned_vector<Eigen::Vector2d>
        solve(ns_eicalib::aligned_vector<Eigen::Vector2d> controlPoints, std::size_t k, std::size_t num) {
    ...
    void GenBsplineExample(const std::string &dir) {
        const ns_eicalib::aligned_vector<Eigen::Vector2d> controlsPoints{
                {0, 1.5},
                {1, 0},
                {2, 2},
                {3, 0},
        ...
        auto writeToFile = [](const ns_eicalib::aligned_vector<Eigen::Vector2d> &ctrlPts,
                              const ns_eicalib::aligned_vector<Eigen::Vector2d> &trajPts,
...
#endif //RTEI_CALIB_BSPLINE_2D_EXAM_HPP
