...
// Revised by lsy on 12/4/24.
//

#ifndef RTEI_CALIB_TRAJECTORY_ESTIMATOR_H
#define RTEI_CALIB_TRAJECTORY_ESTIMATOR_H
...
namespace ns_eicalib {
    ...
    public:
        ...
        enum Option : std::uint32_t {
            ...
            OPT_TIME_OFFSET_IjToIr = 1 << 7,
            OPT_TIME_OFFSET_CkToIr = 1 << 8,
            OPT_TIME_READOUT = 1 << 9,

            OPT_GYRO_BIAS = 1 << 10,
            OPT_GYRO_MAP_COEFF = 1 << 11,

            OPT_ACCE_BIAS = 1 << 12,
            OPT_ACCE_MAP_COEFF = 1 << 13,

            OPT_SO3_AtoG = 1 << 14,

            OPT_GRAVITY_REFINE = 1 << 15,

            OPT_INV_DEPTH = 1 << 16,
            OPT_STRUCTURE_SCALE = 1 << 17,

            OPT_FOCAL_LENGTH = 1 << 18,
            OPT_FOCAL = 1 << 19,
            OPT_DISTO_PARAMS = 1 << 20,

            ALL = OPT_SO3 | OPT_POS | OPT_SO3_IjToIr | OPT_SO3_CkToIr | OPT_POS_IjInIr |
                  OPT_POS_CkInIr | OPT_TIME_OFFSET_IjToIr |  OPT_TIME_OFFSET_CkToIr | OPT_TIME_READOUT |
    ...
    public:
         ...
         */
        void AddIMUGyroMeasurement(const IMUFrame::Ptr &imuFrame, const std::string &imuTopic,
                               std::uint32_t option, double gyroWeight);

        void AddEventAngVelMeasurement(const std::string &eventTopic, const double time_packet,
        ...
        void AddPreIntegrationMeasurementV3(const std::string &camTopic, const Posed &CiToW, const Posed &CjToW,
                                const Posed &C0ToW, const CalibDataManager::Ptr &calibDataManager,
...
#endif //RTEI_CALIB_TRAJECTORY_ESTIMATOR_H
